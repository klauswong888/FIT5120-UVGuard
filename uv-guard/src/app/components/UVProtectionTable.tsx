  ...
  return (
    <div className="bg-white w-full overflow-x-auto shadow-2xl">
      <div className="overflow-hidden min-w-max">
        {/* Table Header */}
        <div
          className="grid p-4 text-sm font-medium text-gray-900 border-t border-b border-gray-200 gap-x-4 dark:bg-gray-800 dark:border-gray-700 dark:text-white"
        ...
        >
          ...
          {config.columns.map((col) => (
            <div key={col.key} className="flex flex-col items-center justify-center">
              {col.icon}
              <span className="text-md mt-1 text-gray-700 dark:text-gray-300">{col.label}</span>
            </div>
          ))}
        </div>
